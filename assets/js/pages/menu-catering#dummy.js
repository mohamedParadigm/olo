$(function () {
  ...
  $("#smarttab").smartTab({
    selected: 0,
    orientation: $(window).width() < 992 ? "horizontal" : "vertical",
    enableURLhash: true,
    transition: {
  ...
  $sticky_nav.find("a").on("click", function (e) {
    ...
    $("html, body").animate(
      {
        scrollTop: $target.offset().top - $sticky_nav.outerHeight() + 1,
      },
      100,
      "linear"
    );
  ...
  $(window).on("scroll", function () {
    $('[data-scrollSpy="block"]').each(function () {
      if (
        $(window).scrollTop() >
        $(this).offset().top - $sticky_nav.outerHeight()
      ) {
  ...
  $(".popup-gallery").magnificPopup({
    ...
    image: {
      ...
      titleSrc: function (item) {
        return `
        <a href="#%curr%" download class="btn btn-alt w-fit download-btn">
          <i>
